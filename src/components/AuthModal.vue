    ...
    <div class="relative w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden">
      ...
      <div class="p-8">
        ...
        <form @submit.prevent="login" class="space-y-4">
          <div>
            <label class="block text-xs font-semibold text-gray-600 mb-1.5 uppercase tracking-wide">Email or Phone</label>
            <input
              v-model="form.email"
              type="text"
              required
              placeholder="you@organization.com or +1234567890"
            ...
            />
            <p class="text-xs text-gray-400 mt-1">Enter your email address or phone number</p>
          </div>
  ...
  methods: {
    async login() {
      ...
      try {
        ...
        this.error = err?.response?.data?.error || err?.response?.data?.detail || err?.response?.data?.message || err?.message || 'Invalid credentials. Please try again.';
      } finally {
        this.loading = false;
      }
    },
